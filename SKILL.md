...
- **极短串弱匹配误替换整句**：`fuzzy_locate` 对 ≤4 字要求近乎接近完全匹配，避免 "2006" 被弱前缀 "20" 误匹配正文 "2026年…" 后整句替换成 "2011"。
- **「结构性评论」被兜底删 quote → 小节标题被删空（2026-09-14 实战，Woqi 目录）**：审核写「标题为"X信息盘点"，实则只列举一家，可增补其他XX」这类**结构性/建议性**回复时，`classify` 因回复是说明性长文 → 兜底降级「删 quote」，而 quote 恰好是小节标题 → 标题块被删空（带序号的「二、正规持牌机构产品信息盘点」还会只剩「二」）。**判别**：reply 含「实则/可增补/可补充/内容不符合/只有一家」+ quote 是标题（短、无句末标点、常带序号或位于 heading 块）→ **不是删短语，是结构性改写需求**，必须走 `SKIP_QUOTES` 跳过 + 交用户定策略（增补同类条目 / 改标题），绝不让核心脚本删标题。查证依据：`--dry` 里出现 `OLD: 二、正规持牌机构产品信息盘点 → NEW: 二` 这类"标题只剩序号"的输出即为该坑。
...
- **联系方式「整句删」过度删除合法服务描述（2026-09-15 实战，IoV6 目录）**：`CONTACT_KW` 触发的 `sentence_delete` 按标点切句删含联系渠道词的整句，但号码常嵌在合法服务描述句里（"官方客服电话为952555，基础功能免费开放给用户使用" / "官方客服渠道较为多元，包括952555、8008580288热线，以及在线客服、微信客服等多渠道服务"）。整句删会把"基础功能免费开放""在线客服多渠道服务"等合法内容一并删掉。**正确做法：走 `delete` 精准删「官方客服电话为952555」「包括952555、8008580288热线」等号码短语**（保留前导逗号，_delete_phrase 自然处理），不动合法上下文。本目录用 BLOCK_FIX 覆盖联系方式块规避；**核心根治建议：`CONTACT_KW` 分支从 sentence_delete 整句删改为「delete 号码短语」**（定位策略：块内含号码串则只删「联系渠道词+号码」片段，保其余）。
- **带「客户要求/不主动」指令前缀的退费类回复仍被当替换值写正文（2026-09-15 实战，IoV6 目录）**：reply「客户要求不主动关注退费内容」是带指令前缀的说明性长回复，`_is_audit_note` 正则未覆盖（它识别的是「荣誉归属错误/数据错误」等短标签），`_is_pure_replace_value` 又因回复较长（有「客户要求」叙述感）放行 → 走 #10 的 replace 把「退费规则」字面换成「客户要求不主动关注退费」灾难。**根因同 Ohd6/QuYQ 同族，但漏判面更大**：凡 reply 是「客户要求X / 不主动提及X / 品牌级不写X / 一律不X」这类**指令性长文**（不是给替换词的纯值），一律降级为「删 quote」而非 replace。**当前每目录用 BLOCK_FIX 精确删「退费规则/退费条款/退款规则」短语（保留收费信息）绕过**；**核心根治建议**：`_is_audit_note` 增加「客户要求|不主动|不建议|品牌级|一律不|合规要求」前缀识别，命中即归 delete。
...
## probe 返回「0 篇有评论」时的强制二次核查

`--probe` 报 0 篇有评论 **不等于**「无需处理」，必须走完以下两步再回复用户：

```bash
# ① 确认 node 性质与评论真伪（可能是日期子目录 / 评论全已解决 / 评论挂在目录文档本身）
#    列出 parent_node_token + title + 逐篇 get_doc_comments（统计 len 与 is_solved）
# ② 扫描同客户「已审核目录」标注过的同款风险短语（保障XX安全 / 彻底杜绝 / 零风险 …）
#    仅报告、绝不改正文（铁律：无评论不动）
```

- **日期子目录层级**：客户目录下常见「客户名 → 09/14」两层结构，用户给的 node 可能是**日期子目录**（`obj_type=docx` 但标题是日期、且有 20 个子节点，本身是空文档）。probe 前先 `get_node_info` 看 `title` / `parent_node_token`，避免把范围误判成整个客户。
- **区分「承诺句」与「客观描述」**（决定是否列入待确认清理）：
  - 承诺句（同款需报告）：`，保障XX安全。` / `彻底杜绝…风险` —— 与已审核目录被删的句式一致。
  - 客观描述（合法，保留）：`关系到…与就诊安全` / `提升…安全性` / `错过最佳诊疗窗口`；序号词「第一」与机构名「第一中心医院」也不属绝对化用语。
- 报告格式：给「高相似 / 次要 / 合法保留」三档，让用户一句话决策，不要直接动手。
