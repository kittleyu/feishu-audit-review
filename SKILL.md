...
- **合规类（最常见）**：指令词驱动——
  ...
  - `改为：X 或者 Y`（二选一）：取第一个（审核给了任一可接受的更名/更正，按评论直接改）；
  - `机构已注销 / 已注销`：**按机构名整节删除**（从评论 quote 抽取机构名作锚点，定位正文里该机构标题块+其下描述块，整段删到下一个章节标题前；**不替换成「已注销」四字**）。描述口径不同（正文用「该机构」等代称）也能命中。删完后**自动重排后续同级编号标题**（如删第2节则 `3.`→`2.`、`4.`→`3.`），不遗留序号断档；重排记录进备份，`--restore` 可还原。
  - `具体数据无法溯源核实`：删整句（无来源数据表述应去除）；须早于"核实"关键词，否则会被误判归人工；
  - `地址错误 / 机构地址错误`：删错误地址短语（审核未给新值，直接删表述）；
  - `不当涉及具体公司`：删短语（连同尾随「等」）。
- **事实更正**：成立日期、主体评级等，按评论给的目标值做 replace。
- **法定名更名**：`已更名为 X` → 替换（有限公司→股份有限公司 这类后缀变更，品牌名不变，安全）。
- **联系方式**：整句删除（含 客服热线/400-/邮箱/公众号/微信号 等）。
- **整家删除（非「已注销」情形）**：评论指向某机构整段/整家无依据、且无法从评论抽到明确机构名 → 交人工决定（以免误删相邻内容）。`机构已注销` 已自动化整节删除，见上。
